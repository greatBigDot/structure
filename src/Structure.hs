...
-- +------------------------------------------+-----------------------------------------------+-----------------------+
-- | \(x+0 = x \)                             | \(x \cdot 1 = x\)                             | /Identities/          |
-- +------------------------------------------+-----------------------------------------------+-----------------------+
-- | \(\exists y \in \mathbb{Z} [ x+y = 0 ]\) |                                               | /(Additive) Inverses/ |
-- +------------------------------------------+-----------------------------------------------+-----------------------+
...
--
-- +------------+------------------------------------------------+-----------------------------+
-- | \(\cdots\) | \(\exists y \in \mathbb{Z} [ x \cdot y = 1 ]\) | /(Multiplicative) Inverses/ |
-- +------------+------------------------------------------------+-----------------------------+
--
...
-- the set of integers modulo five,
-- \(\mathbb{Z}_5 = \left\{\overline{0}, \overline{1}, \overline{2}, \overline{3}, \overline{4}\right\}\).
-- In the set, to calculate the "sum" of two elements, we calculate their usual
...
-- structure becomes just a special case of a more general property, and
-- seemingly disparate things suddenly become two special cases of a more
-- general idea.
--
