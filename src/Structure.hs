...
-- |
-- Module      : Structure
-- Description : The interface to the modules of structure: a representation of various algebraic structures as typeclasses. Laws unverified.
...
-- +------------------------------------------+----------------------------------
-- -------------+-----------------------+
-- | \(x+(y+z) = (x+y)+z\)                    | \(x \cdot (y \cdot z) = (x \cdot
-- y) \cdot z\) | /Associativity/       |
-- +------------------------------------------+----------------------------------
-- -------------+-----------------------+
...
-- discourse, \(\mathbb{Z}\).) There are various things to note. Plenty of other
-- sets satisfy these laws; for example, the rational numbers, or the real
-- numbers, or the complex numbers. Note also that, say, the irrational numbers
-- do /not/ satisfy these properties; \(pi + (1-\pi)\) is rational, and so \(+\)
...
-- necessary, as the following does /not/ hold:
--
-- +------------+------------------------------------------------+---------------
-- --------------+
...
-- not have a multiplicative inverse. After modifying the law to account for
-- that special case, however, the rationals do satisfy it, while the integers
-- still don't.)
--
...
-- key idea, the crucial motivating question of abstract algebra, is the
-- following:
--
...
-- diverse than you may expect. For example, in addition to the set of integers
-- and its extensions, other more esoteric sets satisfy the above laws. E.g.,
-- the set of integers modulo five, \(\mathbb{Z}_5 = \left\{\overline{0},
-- \overline{1}, \overline{2}, \overline{3}, \overline{4}\right\}\). In the set,
-- to calculate the "sum" of two elements, we calculate their usual sum, then
-- take that modulo 5 (that is, the remainder when dividing by 5; the modulo
-- operation is essentially a generalization of the concept of even and odd
-- numbers to divisibilities other than by 2); similarly with the product. Thus,
-- for example, \(\overline{4} + \overline{3} = \overline{2}\). A similar line
-- of reasoning indicates that additive inverses exist---in fact, so do
...
-- its documentation an overview of that algebraic structure and some
...
--
module Structure ( module Structure.Data.Structure
                 ) where

import Structure.Data.Structure
